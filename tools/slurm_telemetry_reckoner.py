...
start_span(start_time=)/end(end_time=) cycle with the same purpose attribute tagging
training uses -- so the emitted spans land in whatever backend the run's inherited
...
def _tag_span(span, purpose=None):
    try:
        span.set_attribute("purpose", purpose or ["goodput"])
