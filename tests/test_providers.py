...
class TestBuildProviders:
    ...
    def test_rank_resource_attributes_use_v01_names(self, monkeypatch):
        ...
        build_providers(
            cfg,
            resource_attributes={NV_DL_RANK: 3, NV_DL_WORLD_SIZE: 16},
...
class TestResourceIdentity:
    ...
    def test_lens_does_not_invent_a_rank(self):
        ...
        assert NV_DL_RANK not in attrs
        assert NV_DL_WORLD_SIZE not in attrs

    def test_caller_supplied_rank_lands_on_the_resource(self):
        cfg = NemoLensConfig(enabled=True, exporter="console", run_id="run1")
        build_providers(cfg, resource_attributes={NV_DL_RANK: 3, NV_DL_WORLD_SIZE: 8})
        attrs = self._resource_attrs()
    ...
    def test_instance_id_derives_from_caller_supplied_rank(self):
        ...
        cfg = NemoLensConfig(enabled=True, exporter="console", run_id="run1")
        build_providers(cfg, resource_attributes={NV_DL_RANK: 3})
    ...
    def test_instance_id_derives_from_rank_zero(self):
        ...
        """
        cfg = NemoLensConfig(enabled=True, exporter="console", run_id="run1")
        build_providers(cfg, resource_attributes={NV_DL_RANK: 0})
    ...
    def test_instance_id_derives_from_rank_zero_via_the_env_channel(self, monkeypatch):
        """The env twin of the above; nv.dl.rank arrives as the string "0"."""
        cfg = NemoLensConfig(enabled=True, exporter="console", run_id="run1")
        monkeypatch.setenv("OTEL_RESOURCE_ATTRIBUTES", f"{NV_DL_RANK}=0")
        build_providers(cfg)
        assert self._resource_attrs()["service.instance.id"] == "run1-rank0"
...
class TestEnvSuppliedIdentity:
    ...
    site to reach, so this is how it supplies nv.dl.rank. Deriving from the caller's
    ...
    def test_instance_id_derives_from_an_env_supplied_rank(self, monkeypatch):
        ...
        monkeypatch.setenv("OTEL_RESOURCE_ATTRIBUTES", "nv.dl.rank=5")
        cfg = NemoLensConfig(enabled=True, exporter="console", run_id="run1")
        build_providers(cfg)
    ...
    def test_an_env_supplied_rank_silences_the_warning(self, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="nemo.lens.providers"):
            build_providers(cfg)
        assert "No nv.dl.rank resource attribute was supplied" not in caplog.text
    ...
    def test_the_sdk_auto_uuid_does_not_suppress_the_derivation(self):
        ...
        """
        cfg = NemoLensConfig(enabled=True, exporter="console", run_id="run1")
        build_providers(cfg, resource_attributes={NV_DL_RANK: 2})
...
class TestMissingRankWarning:
    ...
    def test_warns_when_no_rank_is_supplied(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="nemo.lens.providers"):
            build_providers(cfg)
        assert "No nv.dl.rank resource attribute was supplied" in caplog.text

    def test_silent_when_a_rank_is_supplied(self, caplog):
        cfg = NemoLensConfig(enabled=True, exporter="console", run_id="run1")
        with caplog.at_level(logging.WARNING, logger="nemo.lens.providers"):
            build_providers(cfg, resource_attributes={NV_DL_RANK: 3, NV_DL_WORLD_SIZE: 8})
        assert "nv.dl.rank" not in caplog.text
    ...
    def test_does_not_guess_the_rank_from_the_environment(self, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="nemo.lens.providers"):
            build_providers(cfg)
        assert "No nv.dl.rank resource attribute was supplied" in caplog.text
        assert NV_DL_RANK not in dict(trace.get_tracer_provider().resource.attributes)
