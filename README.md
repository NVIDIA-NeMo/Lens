...
Or let every rank export and filter on the `nv.dl.rank` resource attribute in the
...
`OTEL_RESOURCE_ATTRIBUTES=nv.dl.rank=5,nv.dl.world_size=8`, which lens resolves
alongside `resource_attributes`.

Omitting `nv.dl.rank` from both logs a warning at startup. Without it, telemetry
