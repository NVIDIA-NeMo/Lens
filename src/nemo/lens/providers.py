...
def build_providers(
    ...
    # emitted carried nv.dl.rank the whole time.
    #
...
def _warn_no_rank(attrs, *, explicit_identity: bool) -> None:
    """Report that no ``nv.dl.rank`` reached the resolved resource.
    ...
    * ``OTEL_RESOURCE_ATTRIBUTES`` is the standard OTel resource channel, which
      here happens to carry ``nv.dl.rank`` -- a name lens defines in its own
