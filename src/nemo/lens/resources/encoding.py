...
"""Encoding resource attributes for a child process to inherit.

``OTEL_RESOURCE_ATTRIBUTES`` is the only attribute channel that survives a
``spawn`` or ``exec``, so this is how a worker with no ``setup_telemetry`` call
site of its own gets its identity.
"""
...
__all__ = ["encode_resource_attributes", "OTEL_RESOURCE_ATTRIBUTES"]
