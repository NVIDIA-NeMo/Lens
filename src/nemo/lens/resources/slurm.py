...
def detect_slurm(environ: Mapping[str, str] | None = None) -> dict[str, ResourceAttributeValue]:
    ...
    Allowlisted values from ``OTEL_RESOURCE_ATTRIBUTES`` are returned even when
    ``SLURM_JOB_ID`` is not set. They are authoritative when local values are
...
def _select_slurm_resource_attributes(
    attrs: Mapping[str, str],
) -> dict[str, ResourceAttributeValue]:
    retired = SLURM_RETIRED_RESOURCE_ATTRIBUTE_KEYS
    selected: dict[str, ResourceAttributeValue] = {}
