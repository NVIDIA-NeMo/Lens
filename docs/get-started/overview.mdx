...
- **Rank-agnostic export.** Every process where `config.enabled` is true exports; the caller supplies its own `nv.dl.rank` / `nv.dl.world_size` and decides which ranks report.
...
resource attributes (`nv.dl.rank`, `nv.dl.world_size`). Deciding which ranks report is
the caller's job — set `enabled` per rank, or let every rank export and filter on
`nv.dl.rank` in the collector. See [Production Checklist](../user-guide/production.mdx).
