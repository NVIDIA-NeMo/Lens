...
Each adds the relevant OTel instrumentation package. See [Contrib Helpers](/user-guide/contrib-helpers) for details.
...
The `dev` dependency group includes the SDK, `pytest`, `pytest-cov`, `pre-commit`, and `Ruff`. See [Developer Guide](/developer-guide/contributing).
