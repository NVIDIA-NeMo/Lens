...
See [Sampling](/user-guide/sampling) for how to change which rank(s) export.
...
Before debugging collector plumbing, confirm NeMo Lens itself is producing spans by routing to a file. See [Send telemetry to a file](/backends-demo-stack/send-telemetry-to-a-backend#file). If you see spans in the file, the problem is downstream; if you don't, it's in NeMo Lens or your config.
...
Alternatively, you can extract the context manually. See [Context Propagation](/user-guide/context-propagation).
...
You're calling `setup_telemetry` twice with `config.enabled=True`. This is almost always a bug, as double-init silently breaks telemetry on the OTel SDK, and NeMo Lens fails loudly instead. See [Double-Init Guard](/design/double-init-guard) for the rationale.
...
On non-exporting ranks, `setup_telemetry` sets the enabled group set to empty regardless of config. Nothing in this rank will export, by design. See [Sampling](/user-guide/sampling).
...
Switch from `per_step` or `all` to `default`. Spans in the hot loop (microbatch, communication) are the likely cost. See [Span Groups](/user-guide/span-groups) and [Sampling](/user-guide/sampling) for how to dial this in.
...
See [Custom Exporters](/user-guide/custom-exporters).
