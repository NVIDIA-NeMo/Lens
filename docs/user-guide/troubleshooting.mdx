...
### Cannot Filter By Rank

If you cannot filter or group spans by rank in your backend, or
`service.instance.id` carries no rank, you did not pass `nv.dl.rank`. NeMo Lens
logs this at startup:

```
No nv.dl.rank resource attribute was supplied, so telemetry from this process
