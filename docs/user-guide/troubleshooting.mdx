...
If spans from different ranks are indistinguishable in your backend, or
`service.instance.id` is identical everywhere, you did not pass `dl.rank`. NeMo
Lens logs this at startup:

```
No dl.rank resource attribute was supplied, so telemetry from this process
cannot be told apart from any other rank's ...
```
