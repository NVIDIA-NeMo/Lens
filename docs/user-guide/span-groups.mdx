...
- **Ensure orthogonal controls.** Groups operate independently of which processes are enabled and of the OTel SDK's own `OTEL_TRACES_SAMPLER`. You can enable `per_step` groups, keep 10% of traces at the SDK, and turn telemetry on for only one rank.
