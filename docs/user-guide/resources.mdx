...
| Non-string values | Stringified. `None` values are dropped, not written as `"None"`. |
| An inherited value | Appended to, never re-parsed, so a launcher's bytes pass through untouched. The SDK resolves duplicate keys last-wins, so your keys override inherited ones. |
| Repeated calls | The default `inherited` is a snapshot taken at **import**, so spawning in a loop or re-`exec`ing does not accumulate keys. Pass `inherited=""` to start clean. |
