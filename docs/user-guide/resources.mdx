...
| `service.instance.id` | `"{run_id}-rank{rank}"` when the caller supplies `dl.rank`, otherwise just `"{run_id}"` (and NeMo Lens logs a warning) | `"abc123-rank0"` |
...
a warning is logged at startup: telemetry from this process cannot be told apart
from any other rank's, it cannot be filtered by rank in the collector, and
`service.instance.id` falls back to the run id, which every process in the job
reports identically.
