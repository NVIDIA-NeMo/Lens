...
<Note>
`instrument_fastapi` accepts a `service_name` parameter that is used for the tracer provider resource of the instrumented app. If you have already called `setup_telemetry`, the global service name is still configured there; this parameter lets you override the service name for the FastAPI app only.
