...
`reset_span_groups` empties the `SpanRegistry` and then pins an empty enabled set, in that order — pinning drops the stored spec, so a registration inside the next test cannot reopen groups it did not ask for.
