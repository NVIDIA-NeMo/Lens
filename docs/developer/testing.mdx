...
`OTEL_RESOURCE_ATTRIBUTES` is one of the two channels `build_providers` resolves
identity from, so an ambient value — common on an OTel-instrumented CI runner —
would otherwise supply `nv.dl.rank` or `service.instance.id` to tests written to
assert the no-rank path.
