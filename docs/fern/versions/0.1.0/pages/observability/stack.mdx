...
For production, see [Send Telemetry to a Backend](/backends-demo-stack/send-telemetry-to-a-backend). The same OTLP stream can go to any compliant destination.
...
- **Managed or hosted.** Use [W&B Weave](/backends-demo-stack/send-telemetry-to-a-backend#integrate-with-wb-weave), Grafana Cloud, Honeycomb, Datadog, New Relic, or another hosted backend. See [Send Telemetry to Other Hosted Backends](/backends-demo-stack/send-telemetry-to-a-backend#send-telemetry-to-other-hosted-backends).
- **Self-hosted production.** Use an [OTel Collector](/backends-demo-stack/send-telemetry-to-a-backend#configure-otel-collector) in front of your chosen storage, such as Tempo, Mimir, Loki, Jaeger, or Prometheus at scale.
- **File export for offline analysis.** See [Export Telemetry to a File](/backends-demo-stack/send-telemetry-to-a-backend#export-telemetry-to-a-file).
