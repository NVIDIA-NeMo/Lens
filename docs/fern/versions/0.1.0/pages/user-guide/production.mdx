...
For a full discussion, see [Sampling](/user-guide/sampling). Do not leave this configuration simply because it is the default; instead, select the strategy that best fits the specific run.
...
See [Span Groups](/user-guide/span-groups) for complete details.
...
See [Resource Detection](/user-guide/resource-detection) for the full picture and the `dl.*` versus project-scoped conventions.
...
See [Logging Bridge](/user-guide/logging-bridge).
