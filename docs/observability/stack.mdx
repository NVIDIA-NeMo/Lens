...
For production, see [Send Telemetry to a Backend](../observability/backends.mdx). The same OTLP stream can go to any compliant destination.
...
- **Managed or hosted.** Use [W&B Weave](../observability/backends.mdx#integrate-with-wb-weave), Grafana Cloud, Honeycomb, Datadog, New Relic, or another hosted backend. See [Send Telemetry to Other Hosted Backends](../observability/backends.mdx#send-telemetry-to-other-hosted-backends).
- **Self-hosted production.** Use an [OTel Collector](../observability/backends.mdx#configure-otel-collector) in front of your chosen storage, such as Tempo, Mimir, Loki, Jaeger, or Prometheus at scale.
- **File export for offline analysis.** See [Export Telemetry to a File](../observability/backends.mdx#export-telemetry-to-a-file).
