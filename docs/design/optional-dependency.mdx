...
- `span_attributes(attributes)` → context manager yielding `None`
...
The following are safe to use with fallbacks: `managed_span`, `trace_fn`, `span_cm`, `span_attributes`, `is_span_group_enabled`, `safe_set_span_attributes`, and `SpanRegistry`.
...
Consumers adopting `span_attributes(attributes)` must add its no-op context
manager to their local fallback module. Existing consumers that do not import
this new API need no change. The affected mirror locations, when adopted, are
`Megatron-LM/megatron/core/telemetry/_fallbacks.py`,
`NeMo-RL/nemo_rl/telemetry/_fallbacks.py`, and
`NeMo-Gym/nemo_gym/telemetry/_fallbacks.py`. NVRx uses its own optional adapter.
