...
`setup_telemetry` itself is not thread-safe; it is documented as "call once per process." The [double-init guard](/design/double-init-guard) surfaces violations.
