...
│ — semconv/      attribute and Resource field definitions    │
└─────────────────────────────────────────────────────────────┘
```
...
| `distributed.py` | `broadcast_trace_context`, `create_linked_span` |
| `semconv/` | Attribute names, version tracking, and Resource field definitions |
...
setup_telemetry(config, resource_attributes)
  ...
  │         ├─ resolves service.instance.id from the caller's nv.dl.rank, if given
