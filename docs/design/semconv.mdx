...
- `megatron.*`: Megatron-LM consumer-specific span attributes (e.g., model architecture `megatron.num_layers`/`megatron.hidden_size`, iteration-level `megatron.skipped`/`megatron.update_successful`). These are set as inline string attributes in the Megatron-LM fork and are **not** defined as constants in `nemo.lens.semconv`. The central registry only holds the shared and standard namespaces (`dl.*`, `gen_ai.*`, `gym.*`, `slurm.*`, `nemo.*`, `wandb.*`, `k8s.*`).
- `rl.*` — RL-specific (`reward`, `kl_divergence`, `policy_loss`), **owned by the NeMo-RL consumer**.

  Like `megatron.*`, the `rl.*` names are **not** defined as constants in `nemo.lens.semconv`:
  NeMo-RL declares its `rl.*` series through the metric registry (`register_metric_group`) in its
  own tree. `rl.*` carries RL-scoped counterparts to three quantities the `dl.*` namespace
  registers — `rl.grad_norm`, `rl.learning_rate` and `rl.throughput.tokens_per_sec` — describing the
  *policy update* specifically, so an RL job can report them without claiming the shared `dl.*`
  series that a co-resident trainer owns. The first two are deliberate spelling matches for
  `dl.grad_norm`/`dl.learning_rate`, so the two namespaces differ only by prefix. Throughput is the
  exception: `rl.*` uses the dotted `rl.throughput.tokens_per_sec` to match the
  `rl.response_length.mean`/`rl.generation.duration_ms` subgroup convention, whereas `dl.*` uses the
  flat `dl.throughput_tokens_per_sec`. Query the two by their own names rather than by swapping the
  prefix.
