...
nv.dl.role                   — process role, such as trainer or checkpoint worker
nv.dl.provider.name          — training framework or provider identity
nv.dl.topology.size.{tp,pp,dp}
nv.dl.training.config.*      — stable training configuration
nv.dl.training.target.*      — configured execution targets
nv.dl.software.*             — application-observed software versions
...
- `megatron.*`: Megatron-LM consumer-specific span attributes (e.g., model architecture `megatron.num_layers`/`megatron.hidden_size`, iteration-level `megatron.skipped`/`megatron.update_successful`). These are set as inline string attributes in the Megatron-LM fork and are **not** defined as constants in `nemo.lens.semconv`. The central registry holds the shared and standard namespaces (`dl.*`, `gen_ai.*`, `rl.*`, `gym.*`, `slurm.*`, `nemo.*`, `wandb.*`, `k8s.*`); for `rl.*` it holds the span-attribute names only (see below).
